...
SRC_URI:append:toradex-smarc-imx8mp = " \
    ...
    file://10-toradex-can1-ifname.link \
"
SRC_URI:append:toradex-smarc-imx95 = " \
    file://10-toradex-can0-ifname.link \
    file://10-toradex-can1-ifname.link \
"
...
do_install () {
    ...
    # 10-toradex-can*-ifname.link files are available for Aquila, Lino, OSM, SMARC and Verdin family
    ...
    if [ -f ${S}/10-toradex-can2-ifname.link ]; then
        install -m 0644 ${S}/10-toradex-can2-ifname.link ${D}${sysconfdir}/systemd/network/
    fi
    if [ -f ${S}/10-toradex-can3-ifname.link ]; then
